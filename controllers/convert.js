...
async function calculateConversion(originCurrency,
    originValue,
    targetCurrency) {
  ...
  const converted = {
    ...
    targetValue: (originValue/latestRates[originCurrency]) *
                  latestRates[targetCurrency],
    conversionRate: (latestRates['EUR']/latestRates[originCurrency]) *
                    latestRates[targetCurrency],
  };
  return converted;
}
...
function canBeConverted(allowedCurrencies, originCurrency, targetCurrency) {
  return utils.multipleExist(allowedCurrencies,
